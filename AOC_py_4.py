...
mark = lambda nbd, boards: [[[nb + ((nb == nbd) - (nb[-1] == '*')) * '*' for nb in row] for row in board] for board in
                            boards]

n = 0

while [check(b) for b in boards]==len(boards)*[False]: boards=mark(drawn[n],boards); n+=1
print(int(drawn[n-1])*sunmark(boards[[check(b) for b in boards].index(True)]))  # PART 1
